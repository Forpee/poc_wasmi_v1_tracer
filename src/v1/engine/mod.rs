...
use super::{func::FuncEntityInternal, AsContext, AsContextMut, Func, Signature, tracer::Tracer};
use crate::{
    RuntimeValue,
    Trap,
    TrapKind,
...
impl Engine {
    ...
    pub(crate) fn execute_func_with_trace(
        &mut self,
        ctx: impl AsContextMut,
        func: Func,
        args: &[RuntimeValue],
        ...
        self.inner.lock().execute_func_with_trace(ctx, func, args, results, tracer)
    }
}
...
impl EngineInner {
    ...
    /// - If the given `results` do not match the the length of the expected results of `func`.
    /// - When encountering a Wasm trap during the execution of `func`.
    pub fn execute_func_with_trace(
        &mut self,
        mut ctx: impl AsContextMut,
        func: Func,
        args: &[RuntimeValue],
        ...
        match func.as_internal(&ctx) {
            ...
            FuncEntityInternal::Host(..) => unreachable!()
        }
        Ok(())
    }
